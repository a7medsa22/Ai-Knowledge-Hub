...
export class AiProviderFactory {
  ...
  async getProvider(providerType?: AiProviderType): Promise<AiProvider> {
    const selectedProvider =
      providerType ||
      (this.config.get('AI_PROVIDER') as AiProviderType) ||
      'ollama';
  ...
  private async getFallbackProvider(
    ...
    for (const providerType of fallbackOrder) {
      ...
      try {
        const provider =
          providerType === 'ollama' ? this.ollamaProvider : this.openAiProvider;
        ...
        this.logger.debug(
          `Fallback provider ${providerType} also not available`,
        );
      }
    }
  ...
  async getAvailableProviders(): Promise<string[]> {
    ...
    for (const providerType of providers) {
      try {
        ...
        this.logger.debug(
          `Provider ${providerType} not available: ${error.message}`,
        );
      }
    }
